...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Escape untrusted text before interpolating it into innerHTML. Question and
  // choice text is authored by reviewers / generated by AI, so it must never be
  // treated as live markup (prevents stored XSS in the LMS, e.g. an
  // "<img src=x onerror=...>" payload in a question executing in the player).
  ...
  function showSecurityWarning(message) {
    ...
    overlay.innerHTML = `
      <div style="max-width: 500px;">
        ...
        <p style="font-size: 16px; margin-bottom: 24px; opacity: 0.9;">${escapeHtml(message)}</p>
  ...
  function renderQuestion() {
    ...
    const html = `
      <div class="bg-slate-800 rounded-lg p-6 shadow-xl">
        <h2 class="text-2xl font-bold text-blue-300 mb-4">${escapeHtml(question.text)}</h2>
        <div class="space-y-3">
          ${shuffledChoices
            .map(
              (choice, index) => `
            ...
            >
              ...
              <span class="font-semibold">${escapeHtml(choice.text)}</span>
            </button>
          `
            )
            .join("")}
