...
describe("fileProcessor", () => {
  describe("detectLanguageFromFilename", () => {
    it("detects language from full name", () => {
      expect(
        detectLanguageFromFilename("questions_Chinese_(Simplified).csv")
  ...
  describe("parseCSVQuestions", () => {
    ...
    it("parses v1.7 format correctly", () => {
      const row =
        "1,uid123,accepted,Art,Easy,Multiple Choice,Q1,A,B,C,D,A,Exp,English,http://url,2023-01-01";
    ...
    it("detects language from filename if missing in row (v1.7)", () => {
      ...
      const row =
        "1,uid123,accepted,Art,Easy,Multiple Choice,Q1,A,B,C,D,A,Exp,,http://url,2023-01-01";
      const content = `${v17Header}\n${row}`;

      const result = parseCSVQuestions(
        content,
        "questions_Chinese_(Simplified).csv",
        "DefaultCreator"
      );
