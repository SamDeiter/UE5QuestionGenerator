...
export const filterDuplicateQuestions = (
  ...
  if (uniqueNew.length < newItems.length) {
    ...
    console.log(
      `[Filter] Removed ${removed} duplicate(s) that already exist in question database`
    );
  }
...
export const parseQuestions = (text) => {
  ...
  // Aggressively clean markdown code blocks (handle both \r\n and \n line endings)
  ...
  dataLines.forEach((line, index) => {
    ...
    if (type === "True/False") {
      ...
      if (isMalformed) {
        console.warn(
          `[Parser] Rejected malformed MC question with single-letter option: "${question.substring(
            0,
            50
          )}..."`
        );
