  ...
  return (
    <div className="fixed inset-0 bg-black/90 z-[200] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-700 p-6 rounded-xl shadow-2xl max-w-md w-full animate-in zoom-in-95 duration-300 space-y-4">
        <h3
          className={`text-xl font-bold flex items-center gap-2 ${isDanger ? "text-red-500" : "text-yellow-500"}`}
        >
        ...
        {expectedValue && (
          <p className="text-xs text-slate-500">
            Type <span className="font-bold text-red-400">{expectedValue}</span>{" "}
            to confirm
          </p>
        )}
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button
            ...
            className={`flex-1 px-4 py-2 text-sm rounded font-bold transition-colors ${
              isDanger
