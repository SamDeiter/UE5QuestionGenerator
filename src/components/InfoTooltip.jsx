...
const InfoTooltip = ({ text, direction = "up" }) => (
  <div className="group relative inline-flex items-center ml-1.5 align-middle">
    <Icon
      name="info"
      size={12}
      className="text-slate-600 hover:text-orange-500 cursor-help transition-colors"
    />
    <div
    ...
    >
      {text}
      <div
        className={`absolute left-1/2 -translate-x-1/2 border-4 border-transparent ${direction === "down" ? "bottom-full border-b-slate-800" : "top-full border-t-slate-800"}`}
      ></div>
    </div>
  </div>
);
