...
import { mockCSVData, createMockFile, mockQuestions } from "./testHelpers";
...
describe("Import/Export Integration Tests", () => {
  ...
  describe("CSV Import", () => {
    it("should import valid CSV file successfully", async () => {
      const csvContent = `ID,UniqueId,Discipline,Type,Difficulty,Question,OptionA,OptionB,OptionC,OptionD,CorrectLetter,DateAdded,SourceURL,SourceExcerpt,SourceVerified,HumanVerified,HumanVerifiedAt,HumanVerifiedBy,CreatorName,ReviewerName,Language
1,uid-001,Graphics,Multiple Choice,Easy,What is UE5?,Game Engine,Car,Food,Planet,A,2024-01-01,,,,,,,TestUser,,English`;
      ...
      if (result.type !== "questions")
        logger.error("Unexpected Result Type:", result.type, result);
...
1,uid-002,Graphics,Multiple Choice,Easy,什么是UE5?,A,B,C,D,A,2024-01-01,,,,,,,TestUser,,Chinese`;
  ...
  describe("CSV Export", () => {
    it("should generate valid CSV content from questions", () => {
      const questions = [mockQuestions.multipleChoice, mockQuestions.trueFalse];
    ...
    it("should exclude rejected questions when filtered before export", () => {
      const questions = [mockQuestions.multipleChoice, mockQuestions.rejected];
    ...
    it("should handle special characters in CSV fields", () => {
      const specialQuestion = {
        ...
        options: {
          ...mockQuestions.multipleChoice.options,
