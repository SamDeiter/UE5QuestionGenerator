...
def check_english_ids():
    docs = db.collection('questions').where(filter=firestore.FieldFilter('language', '==', 'English')).limit(20).get()
    print(f"Checking {len(docs)} English documents:")
    for d in docs:
        data = d.to_dict()
        print(f"ID: {d.id}, UniqueId: {data.get('uniqueId')}")
